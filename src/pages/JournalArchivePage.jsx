...
export default function JournalArchivePage() {
  ...
  const { kids, updateKid } = useKids(familyId)
  const { journals, loading, addJournal, updateJournal, deleteJournal } = useJournals(familyId, childId)
    ...
    <div className="min-h-screen bg-cream flex">
      ...
      {showModal && (
        <JournalEntryModal
          entry={editingEntry}
          childId={childId}
          childName={kid?.name || 'your child'}
          onClose={() => { setShowModal(false); setEditingEntry(null) }}
          onSave={editingEntry ? updateJournal : addJournal}
        />
      )}
      </div>
    </div>
